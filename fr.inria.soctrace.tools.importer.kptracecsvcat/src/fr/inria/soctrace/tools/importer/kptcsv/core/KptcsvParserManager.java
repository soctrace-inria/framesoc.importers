...
public class KptcsvParserManager {
	...
	public int parseTrace() throws FileNotFoundException, SoCTraceException {
		...
		// TODO : verify if file is correct : not empty and 1st line contains
		// names
		...
		while (tracescan.hasNextLine()) {
			...
			if (listEventProducerName.contains(tokens[5])) {
				e.setEventProducer(listEventProducer.get(listEventProducerName
						.indexOf(tokens[5])));
			} else {
				EventProducer eprod = new EventProducer(
						eprodIdManager.getNextId());
				eprod.setName(tokens[5]);
				eprod.setType(tokens[5]);
				eprod.setLocalId(tokens[5]);
				traceDB.save(eprod);
				listEventProducerName.add(listEventProducerName.size(),
						tokens[5]);
			...
			if (listEventTypeNames.contains(tokens[4])) {
				// manage bug from the trace that have for one type two category
				// (ex: sys_select)
				if (e.getCategory() != listEventType.get(
						listEventTypeNames.indexOf(tokens[4])).getCategory()) {
					continue;
				}
				e.setType(listEventType.get(listEventTypeNames
						.indexOf(tokens[4])));
			} else {
				...
				for (int i = 8; i < names.length; i++) {
					if (tokens[i].length() != 0) {
						EventParamType ept = new EventParamType(
								eptIdManager.getNextId());
						ept.setEventType(et);
						ept.setName(names[i]);
						ept.setType("STRING");
						traceDB.save(ept);
			...
			for (int i = 8; i < names.length; i++) {
				if (tokens[i].length() != 0) {
					...
					ep.setEvent(e);
					ep.setEventParamType(listEventParamType.get(
							e.getType().getId()).get(paramnumber));
