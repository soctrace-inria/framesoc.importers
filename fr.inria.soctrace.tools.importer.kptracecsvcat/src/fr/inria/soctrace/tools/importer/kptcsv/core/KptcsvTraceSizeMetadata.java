...
public class KptcsvTraceSizeMetadata {
	...
	public KptcsvTraceSizeMetadata(File input, String traceDbName)
		...
		tQuery.setElementWhere(new SimpleCondition("TRACE_DB_NAME",
				ComparisonOperation.EQ, traceDB.getDBName()));
		List<Trace> traces = tQuery.getList();
		if (!traces.isEmpty()) {
			saveParam(traces.get(0), sysDB, "Trace size", "String",
					getStringSize(input));
			traceDB.close();
			sysDB.close();
			...
			String dbms = Configuration.getInstance().get(
					SoCTraceProperty.soctrace_dbms);
			if (dbms.equalsIgnoreCase(DBMS.SQLITE.toString())) {
				value = getStringSize(new File(Configuration.getInstance().get(
						SoCTraceProperty.sqlite_db_directory)
						+ File.separator + traceDB.getDBName()));
			}
	...
	private void saveParam(Trace trace, SystemDBObject sysDB, String name,
			String type, String value) throws SoCTraceException {
		...
		try {
			...
			tptId.setNextId(1 + sysDB.getMaxId(
					FramesocTable.TRACE_PARAM_TYPE.toString(), "ID"));
			tpId.setNextId(1 + sysDB.getMaxId(
					FramesocTable.TRACE_PARAM.toString(), "ID"));

			Map<String, TraceParamType> tptMap = typeMap.get(trace.getType()
					.getId());
			if (!tptMap.containsKey(name)) {
				// the trace param type is not present for this trace type:
				// create it
			...
			if (!trace.getParamMap().containsKey(name)) {
				// the trace param is not present for this trace param type:
				// create it
